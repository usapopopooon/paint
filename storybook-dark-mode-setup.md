# Storybook 10でダークモードをセットアップしてみた

Storybook 10でダークモードを設定する機会があったので、やり方をメモしておきます。Storybook全体をダークテーマに統一するまでの手順です。もっと良いやり方があるかもしれませんが、参考程度にどうぞ。

ダークモードの動作確認用として、Tailwind CSSの`dark:`バリアントに対応したコンポーネントがあると便利です。今回はshadcn/uiのButtonコンポーネントを使って確認していきます。
...
ツールバーでライトモードに切り替えると、プレビュー領域の背景とコンポーネントのスタイルが切り替わります。先述の通り、残念ながらUIクロームはダーク固定になります。

| ダークモード | ライトモード |
|:---:|:---:|
| ![ダークモード](/images/xxxxx/storybook-dark.png) | ![ライトモード](/images/xxxxx/storybook-light.png) |
...
公式ドキュメントを読みながら試行錯誤した結果なので、もしかしたらもっとスマートなやり方があるかもしれません。何かあればコメントで教えていただけると嬉しいです。