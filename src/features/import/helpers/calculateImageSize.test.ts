...
describe('calculateImageSize', () => {
  ...
  describe('画像がキャンバスより小さい場合', () => {
    test('拡大せずに中央配置される', () => {
      ...
      // 中央配置
      expect(result.x).toBe((canvasWidth - result.width) / 2)
      expect(result.y).toBe((canvasHeight - result.height) / 2)
    })
  ...
  describe('画像の幅がキャンバスより大きい場合', () => {
    test('幅に合わせて縮小される', () => {
      const imageWidth = 1600 // canvasWidth(800)の2倍
      const imageHeight = 300
  ...
  describe('画像の高さがキャンバスより大きい場合', () => {
    test('高さに合わせて縮小される', () => {
      const imageWidth = 400
      const imageHeight = 1200 // canvasHeight(600)の2倍
  ...
  describe('画像の両辺がキャンバスより大きい場合', () => {
    test('より制約が厳しい方に合わせて縮小される（幅が制約）', () => {
      const imageWidth = 2400 // 幅のスケールがより小さい
    ...
    test('より制約が厳しい方に合わせて縮小される（高さが制約）', () => {
      ...
      const imageHeight = 2000 // 高さのスケールがより小さい
